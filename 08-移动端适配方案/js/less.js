...
Comment.prototype.genCSS = function (context, output) {
    if (this.debugInfo) {
        output.add(getDebugInfo(context, this), this.currentFileInfo, this.index1);
    }
    output.add(this.value);
};
...
JsEvalNode.prototype.evaluateJavaScript = function (expression, context) {
    ...
    expression = expression.replace(/@\{([\w-]+)\}/g, function (_, name) {
        return that.jsify(new Variable('@' + name, that.index1, that.currentFileInfo).eval(context));
    });
...
var Definition = function (name, params, rules, condition, variadic, frames) {
    this.name = name;
    this.selectors = [new Selector([new Element(null, name, this.index1, this.currentFileInfo)])];
    this.params = params;
...
Ruleset.prototype.joinSelector = function (paths, context, selector) {
    ...
    function createParenthesis(elementsToPak, originalElement) {
        ...
        if (elementsToPak.length === 0) {
            ...
            for (j = 0; j < elementsToPak.length; j++) {
                insideParent.push(new Element(null, elementsToPak[j], originalElement.index1, originalElement.currentFileInfo));
            }
    ...
    function createSelector(containedElement, originalElement) {
        ...
        element = new Element(null, containedElement, originalElement.index1, originalElement.currentFileInfo);
            ...
            if (el.value !== "&") {
                ...
                for (j = 0; j < newSelectors.length; j++) {
                    ...
                    if (context.length === 0) {
                        ...
                        // it is not lost
                        if (sel.length > 0) {
                            sel[0].elements.push(new Element(el.combinator, '', el.index1, el.currentFileInfo));
                        }
    ...
    function addReplacementIntoPath(beginningPath, addPath, replacedElement, originalSelector) {
        ...
        if (addPath.length > 0) {
            ...
            newJoinedSelector.elements.push(new Element(combinator, parentEl.value, replacedElement.index1, replacedElement.currentFileInfo));
...
Selector.prototype.createDerived = function(elements, extendList, evaldCondition) {
    ...
    var newSelector = new Selector(elements, extendList || this.extendList, null, this.index1, this.currentFileInfo, this.isReferenced);
    newSelector.evaldCondition = evaldCondition;
    newSelector.mediaEmpty = this.mediaEmpty;
    return newSelector;
};
Selector.prototype.createEmptySelectors = function() {
    var el = new Element('', '&', this.index1, this.currentFileInfo),
        sels = [new Selector([el], null, null, this.index1, this.currentFileInfo)];
...
Selector.prototype.genCSS = function (context, output) {
    ...
    if ((!context || !context.firstSelector) && this.elements[0].combinator.value === "") {
        output.add(' ', this.currentFileInfo, this.index1);
    }
    if (!this._css) {
    ...
    checkExtendsForNonMatched: function(extendList) {
        ...
        extendList.filter(function(extend) {
            return !extend.hasFoundMatches && extend.parent_ids.length == 1;
                ...
                if (!indicies[extend.index1 + ' ' + selector]) {
                    indicies[extend.index1 + ' ' + selector] = true;
    ...
    extendSelector:function (matches, selectorPath, replacementSelector) {
        ...
        for (matchIndex = 0; matchIndex < matches.length; matchIndex++) {
            ...
            firstElement = new tree.Element(
                ...
                replacementSelector.elements[0].index1,
...
ImportVisitor.prototype = {
    ...
    processImportNode: function(importNode, context, importParent) {
        ...
        try {
            ...
            if (!e.filename) { e.index = importNode.index1; e.filename = importNode.currentFileInfo.filename; }
    ...
    onImported: function (importNode, context, e, root, importedAtRoot, fullPath) {
        if (e) {
            if (!e.filename) {
                e.index = importNode.index1; e.filename = importNode.currentFileInfo.filename;
            }
            this.error = e;
        }
        ...
        for (var i = 0; i < rules.length; i++) {
            ...
            if (ruleNode instanceof tree.Rule && !ruleNode.variable) {
                throw { message: "properties must be inside selector blocks, they cannot be in the root.",
                    index: ruleNode.index1, filename: ruleNode.currentFileInfo ? ruleNode.currentFileInfo.filename : null};
            }
        }
    },
